...
contract GnosisSafeAccountFactory {
    ...
    function createAccount(address owner, uint threshold, uint256 salt) public returns (address) {
        address addr = getAddress(owner, threshold, salt);
    ...
    function getInitializer(address owner, uint threshold) internal view returns (bytes memory) {
     ...
     */
    function getAddress(address owner, uint threshold, uint256 salt) public view returns (address) {
        bytes memory initializer = getInitializer(owner, threshold);
